...
var customerList,
    perPage = 10,
    options = { valueNames: ["reasons", "nooftransaction"], page: perPage, pagination: !0, plugins: [ListPagination({ left: 2, right: 2 })] };
...
var suspectedAadhaar,
    perPage = 10,
    options = { valueNames: ["aadhaar", "nooftransaction","noofsuccesstransaction"], page: perPage, pagination: !0, plugins: [ListPagination({ left: 2, right: 2 })] };
document.getElementById("suspectedAadhaar") &&
    (suspectedAadhaar = new List("suspectedAadhaar", options).on("updated", function (e) {
        0 == e.matchingItems.length ? (document.getElementsByClassName("noresult")[0].style.display = "block") : (document.getElementsByClassName("noresult")[0].style.display = "none");
        var t = 1 == e.i,
            a = e.i > e.matchingItems.length - e.page;
        document.querySelector(".pagination-prev.disabled") && document.querySelector(".pagination-prev.disabled").classList.remove("disabled"),
...
(xhttp.onload = function () {
    ...
    xhttp.send(),
    (isCount = new DOMParser().parseFromString(customerList.items.slice(-1)[0]._values.id, "text/html"));
...
function deleteMultiple() {
    ...
    } else Swal.fire({ title: "Please select at least one checkbox", confirmButtonClass: "btn btn-info", buttonsStyling: !1, showCloseButton: !0 });
}
...
var attroptions = { valueNames: ["name", "born", { data: ["id"] }, { attr: "src", name: "image" }, { attr: "href", name: "link" }, { attr: "data-timestamp", name: "timestamp" }] },
...
var existOptionsList = { valueNames: ["contact-name", "contact-message"] },
    ...
    paginationList = new List("pagination-list", { valueNames: ["pagi-list"], page: 3, pagination: !0 }); */
